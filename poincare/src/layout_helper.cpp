...
Layout LayoutHelper::Infix(const Expression& expression,
  ...
  for (int i = 0; i < numberOfChildren; i++) {
    ...
    if (i > 0) {
      ...
      Layout operatorLayout = operatorLayoutBuilder(operatorName, leftChild,
                                                    rightChild, childLayout);
      if (!operatorLayout.isUninitialized()) {
        result.addOrMergeChildAtIndex(operatorLayout,
...
Layout LayoutHelper::Prefix(const Expression& expression,
  ...
  for (int i = 0; i < numberOfChildren; i++) {
    if (i > 0) {
      args.addChildAtIndexInPlace(CodePointLayout::Builder(','),
                                  args.numberOfChildren(),
                                  args.numberOfChildren());
    }
    args.addOrMergeChildAtIndex(
        expression.childAtIndex(i).createLayout(
            floatDisplayMode, numberOfSignificantDigits, context, false, true),
        args.numberOfChildren());
  }
