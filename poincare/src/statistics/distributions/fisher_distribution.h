#ifndef POINCARE_STATISTICS_PROBABILITY_FISHER_DISTRIBUTION_H
#define POINCARE_STATISTICS_PROBABILITY_FISHER_DISTRIBUTION_H
...
class FisherDistribution final : public ContinuousDistribution {
 public:
  ...
  template <typename T>
  T CumulativeDistributiveInverseForProbability(T probability, const T d1,
                                                const T d2) const;
  float cumulativeDistributiveInverseForProbability(
      float x, const float* parameters) const override {
    return this->CumulativeDistributiveInverseForProbability<float>(
