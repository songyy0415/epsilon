...
RationalNode::RationalNode(const native_uint_t *numeratorDigits,
                           uint8_t numeratorSize,
                           const native_uint_t *denominatorDigits,
                           uint8_t denominatorSize, bool negative)
    : m_negative(negative),
      m_numberOfDigitsNumerator(numeratorSize),
      m_numberOfDigitsDenominator(denominatorSize) {
  assert(numeratorSize == 0 ||
         (numeratorDigits == nullptr) ==
             (numeratorSize > Integer::k_maxNumberOfDigits));
  if (numeratorDigits) {
    memcpy(m_digits, numeratorDigits, numeratorSize * sizeof(native_uint_t));
  } else {
    numeratorSize = 0;
  }
  assert(denominatorSize == 0 ||
         (denominatorDigits == nullptr) ==
             (denominatorSize > Integer::k_maxNumberOfDigits));
  if (denominatorDigits) {
    memcpy(m_digits + numeratorSize, denominatorDigits,
           denominatorSize * sizeof(native_uint_t));
  }
}

Integer RationalNode::signedNumerator() const {
  return Integer::BuildInteger((native_uint_t *)m_digits,
                               m_numberOfDigitsNumerator, m_negative, false);
}

Integer RationalNode::unsignedNumerator() const {
  return Integer::BuildInteger((native_uint_t *)m_digits,
                               m_numberOfDigitsNumerator, false, false);
}

Integer RationalNode::denominator() const {
  uint8_t numeratorSize =
      m_numberOfDigitsNumerator > Integer::k_maxNumberOfDigits
          ? 0
          : m_numberOfDigitsNumerator;
  return Integer::BuildInteger(((native_uint_t *)m_digits + numeratorSize),
                               m_numberOfDigitsDenominator, false, false);
}

// Tree Node

static size_t RationalSize(uint8_t numeratorNumberOfDigits,
                           uint8_t denominatorNumberOfDigits) {
  uint8_t realNumeratorSize =
      numeratorNumberOfDigits > Integer::k_maxNumberOfDigits
          ? 0
          : numeratorNumberOfDigits;
  uint8_t realDenominatorSize =
      denominatorNumberOfDigits > Integer::k_maxNumberOfDigits
          ? 0
          : denominatorNumberOfDigits;
  return sizeof(RationalNode) +
         sizeof(native_uint_t) * (realNumeratorSize + realDenominatorSize);
}
...
size_t RationalNode::serialize(char *buffer, size_t bufferSize,
  ...
  numberOfChar +=
      denominator().serialize(buffer + numberOfChar, bufferSize - numberOfChar);
  return numberOfChar;
}
...
int RationalNode::simplificationOrderSameType(const ExpressionNode *e,
                                              bool ascending,
...
Rational Rational::Builder(Integer &num, Integer &den) {
  ...
  bool negative = (!num.isNegative() && den.isNegative()) ||
                  (!den.isNegative() && num.isNegative());
  return Rational::Builder(num.digits(), num.numberOfDigits(), den.digits(),
                           den.numberOfDigits(), negative);
}

Rational Rational::Builder(const Integer &numerator) {
  native_uint_t one = 1;
  return Rational::Builder(numerator.digits(), numerator.numberOfDigits(), &one,
                           1, numerator.isNegative());
}
...
bool Rational::numeratorOrDenominatorIsInfinity() const {
  return signedIntegerNumerator().isOverflow() ||
         integerDenominator().isOverflow();
}
...
Rational Rational::Addition(const Rational &i, const Rational &j) {
  Integer newNumerator =
      Integer::Addition(Integer::Multiplication(i.signedIntegerNumerator(),
                                                j.integerDenominator()),
                        Integer::Multiplication(j.signedIntegerNumerator(),
                                                i.integerDenominator()));
  Integer newDenominator =
      Integer::Multiplication(i.integerDenominator(), j.integerDenominator());
  return Rational::Builder(newNumerator, newDenominator);
}

Rational Rational::Multiplication(const Rational &i, const Rational &j) {
  Integer newNumerator = Integer::Multiplication(i.signedIntegerNumerator(),
                                                 j.signedIntegerNumerator());
  Integer newDenominator =
      Integer::Multiplication(i.integerDenominator(), j.integerDenominator());
  return Rational::Builder(newNumerator, newDenominator);
}
...
Rational Rational::Builder(const native_uint_t *i, uint8_t numeratorSize,
                           const native_uint_t *j, uint8_t denominatorSize,
                           bool negative) {
  void *bufferNode =
      TreePool::sharedPool->alloc(RationalSize(numeratorSize, denominatorSize));
