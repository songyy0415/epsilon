...
void removeZeroAtTheEnd(Integer *i, int minimalNumbersOfDigits = -1) {
  ...
  while (d.remainder.isZero()) {
    if (shouldCheckMinimalNumberOfDigits &&
        (Integer::NaturalOrder(d.quotient, minimum) < 0 &&
         Integer::NaturalOrder(d.quotient, minusMinimum) > 0)) {
      break;
    }
    *i = d.quotient;
...
DecimalNode::DecimalNode(const native_uint_t *mantissaDigits,
                         uint8_t mantissaSize, int exponent, bool negative)
    : m_negative(negative),
      m_exponent(exponent),
...
Integer DecimalNode::signedMantissa() const {
  return Integer::BuildInteger((native_uint_t *)m_mantissa,
                               m_numberOfDigitsInMantissa, m_negative);
}

Integer DecimalNode::unsignedMantissa() const {
  return Integer::BuildInteger((native_uint_t *)m_mantissa,
                               m_numberOfDigitsInMantissa, false);
}
...
int DecimalNode::simplificationOrderSameType(const ExpressionNode *e,
  ...
  if (m_exponent < other->m_exponent) {
    ...
    unsignedComparison =
        Integer::NaturalOrder(unsignedMantissa(), other->unsignedMantissa());
  }
  ...
  if (integralPart == integralPartEnd) {
    ...
    if (fractionalPart != nullptr) {
      while (UTF8Helper::CodePointIs(fractionalPart, '0') &&
             fractionalPart < fractionalPartEnd) {
        fractionalPart++;
        exp--;
      }
    }
...
Decimal Decimal::Builder(const char *integralPart, int integralPartLength,
                         const char *fractionalPart, int fractionalPartLength,
                         int exponent) {
  ...
  if (fractionalPartLength == 0) {
    while (
        ...
        UTF8Helper::CodePointIs(&integralPart[integralPartLength - 1], '0')) {
      integralPartLength--;
    }
  }
  ...
  for (int i = 0; i < fractionalPartLength; i++) {
    ...
    incrementExponentAfterRoundingUp =
...
Decimal Decimal::Builder(T f) {
  ...
  constexpr int numberOfSignificantDigits =
  ...
  mantissaf = mantissaf *
              std::pow((double)10.0, (double)(numberOfSignificantDigits - 1));
  /* If m > 99999999999999.5 or 9999999,5, the mantissa stored will be 1 (as we
   * keep only 14 significative numbers from double. In that case, the exponent
   * must be increment as well. */
  const double biggestMantissaFromDouble =
      std::pow((double)10.0, (double)(numberOfSignificantDigits)) - 0.5;
