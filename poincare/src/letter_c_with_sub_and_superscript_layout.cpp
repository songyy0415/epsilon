...
void LetterCWithSubAndSuperscriptLayoutNode::renderLetter(
  ...
  for (int i = 0; i < k_symbolHeight / 2; i++) {
    for (int j = 0; j < k_symbolWidth; j++) {
      symbolPixel[i * k_symbolWidth + j] =
          symbolUpperHalf[i * k_symbolWidth + j];
      // The bottom half is obtained by mirroring the upper half
      symbolPixel[(k_symbolHeight - 1 - i) * k_symbolWidth + j] =
          symbolUpperHalf[i * k_symbolWidth + j];
    }
  }
