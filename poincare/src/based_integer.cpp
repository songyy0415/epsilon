...
BasedIntegerNode::BasedIntegerNode(const native_uint_t *digits, uint8_t size,
                                   OMG::Base base)
    : NumberNode(), m_base(base), m_numberOfDigits(size) {
  if (digits) {
...
Integer BasedIntegerNode::integer() const {
  return Integer::BuildInteger((native_uint_t *)m_digits, m_numberOfDigits,
                               false);
}
...
static size_t BasedIntegerSize(uint8_t numberOfDigits) {
  uint8_t realDigitsSize =
      numberOfDigits > Integer::k_maxNumberOfDigits ? 0 : numberOfDigits;
...
BasedInteger BasedInteger::Builder(const char *digits, size_t length,
                                   OMG::Base base) {
