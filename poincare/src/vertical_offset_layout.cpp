...
VerticalOffsetLayoutNode::deletionMethodForCursorLeftOfChild(
    int childIndex) const {
  assert(childIndex == 0 || childIndex == k_outsideIndex);
  return childIndex == 0 && childAtIndex(0)->isEmpty()
...
int VerticalOffsetLayoutNode::indexAfterVerticalCursorMove(
...
KDSize VerticalOffsetLayoutNode::computeSize(KDFont::Size font) {
  ...
  if (verticalPosition() == VerticalPosition::Superscript) {
    ...
    if (idxInParent < parentNode->numberOfChildren() - 1 &&
        parentNode->childAtIndex(idxInParent + 1)->hasUpperLeftIndex()) {
      width += k_separationMargin;
    }
  }
  if (m_emptyBaseVisibility == EmptyRectangle::State::Visible &&
      baseLayout() == nullptr) {
    width += EmptyRectangle::RectangleSize(font).width();
  }
  KDCoordinate height = baseSize(font).height() - k_indiceHeight +
                        indiceLayout()->layoutSize(font).height();
...
KDCoordinate VerticalOffsetLayoutNode::computeBaseline(KDFont::Size font) {
  ...
  assert(verticalPosition() == VerticalPosition::Superscript);
  return indiceLayout()->layoutSize(font).height() - k_indiceHeight +
         baseBaseline(font);
}
...
KDPoint VerticalOffsetLayoutNode::positionOfChild(LayoutNode *child,
  ...
  KDCoordinate shiftForEmpty =
      baseLayout() == nullptr &&
              horizontalPosition() == HorizontalPosition::Suffix
          ? baseSize(font).width()
          : 0;
  if (verticalPosition() == VerticalPosition::Superscript) {
...
KDSize VerticalOffsetLayoutNode::baseSize(KDFont::Size font) {
  return baseLayout()
             ? baseLayout()->layoutSize(font)
             : (m_emptyBaseVisibility == EmptyRectangle::State::Visible
                    ? EmptyRectangle::RectangleSize(font)
                    : KDSize(0, EmptyRectangle::RectangleSize(font).height()));
}

KDCoordinate VerticalOffsetLayoutNode::baseBaseline(KDFont::Size font) {
  return baseLayout() ? baseLayout()->baseline(font)
