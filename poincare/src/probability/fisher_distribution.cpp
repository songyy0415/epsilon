...
#include <float.h>
#include <omg/float.h>
#include <poincare/probability/fisher_distribution.h>
...
template <typename T>
T FisherDistribution::CumulativeDistributiveFunctionAtAbscissa(T x, T d1,
                                                               T d2) {
...
template <typename T>
T FisherDistribution::CumulativeDistributiveInverseForProbability(T probability,
                                                                  T d1,
                                                                  T d2) const {
  if (!D1AndD2AreOK(d1, d2)) {
    return NAN;
  ...
  return cumulativeDistributiveInverseForProbabilityUsingIncreasingFunctionRoot(
      probability, DBL_EPSILON, 100.0, parameters);  // Ad-hoc value;
}

template <typename T>
bool FisherDistribution::D1AndD2AreOK(T d1, T d2) {
  return Domain::Contains(d1, Domain::Type::RPlusStar) &&
...
template float FisherDistribution::EvaluateAtAbscissa<float>(float, float,
                                                             float);
template double FisherDistribution::EvaluateAtAbscissa<double>(double, double,
                                                               double);
template float
FisherDistribution::CumulativeDistributiveFunctionAtAbscissa<float>(float,
                                                                    float,
                                                                    float);
template double
FisherDistribution::CumulativeDistributiveFunctionAtAbscissa<double>(double,
                                                                     double,
                                                                     double);
template float FisherDistribution::CumulativeDistributiveInverseForProbability<
    float>(float, float, float) const;
template double FisherDistribution::CumulativeDistributiveInverseForProbability<
