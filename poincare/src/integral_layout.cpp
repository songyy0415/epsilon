...
const uint8_t topSymbolPixel[IntegralLayoutNode::k_symbolHeight]
                            [IntegralLayoutNode::k_symbolWidth] = {
                                {0xFF, 0xD5, 0x46, 0x09},
                                {0xF1, 0x1A, 0x93, 0xF0},
                                {0x98, 0x54, 0xFF, 0xFF},
                                {0x53, 0xA7, 0xFF, 0xFF},
...
const uint8_t bottomSymbolPixel[IntegralLayoutNode::k_symbolHeight]
                               [IntegralLayoutNode::k_symbolWidth] = {
                                   {0xFF, 0xFF, 0xFF, 0x00},
                                   {0xFF, 0xFF, 0xFF, 0x00},
                                   {0xFF, 0xFF, 0xFE, 0x03},
...
int IntegralLayoutNode::indexAfterHorizontalCursorMove(
  ...
  switch (currentIndex) {
    ...
    case k_integrandLayoutIndex:
      return direction.isRight() ? k_differentialLayoutIndex
                                 : k_lowerBoundLayoutIndex;
    default:
...
IntegralLayoutNode::deletionMethodForCursorLeftOfChild(int childIndex) const {
...
KDSize IntegralLayoutNode::computeSize(KDFont::Size font) {
  ...
  if (this == last) {
    height = k_boundVerticalMargin +
             ...
             k_integrandVerticalMargin + centralArgumentHeight(font) +
             k_integrandVerticalMargin +
             boundMaxHeight(BoundPosition::LowerBound, font) +
             k_boundVerticalMargin;
  } else {
...
KDPoint IntegralLayoutNode::positionOfChild(LayoutNode *child,
  ...
  if (child == lowerBoundLayout()) {
    x = boundOffset;
    y = computeSize(font).height() - k_boundVerticalMargin -
        boundMaxHeight(BoundPosition::LowerBound, font);
    ...
    x = boundOffset + std::max(lowerBoundSize.width(), upperBoundSize.width()) +
        k_integrandHorizontalMargin;
    y = computeBaseline(font) - integrandLayout()->baseline(font);
  } else {
    assert(child == differentialLayout());
    x = computeSize(font).width() -
        differentialLayout()->layoutSize(font).width();
...
KDCoordinate IntegralLayoutNode::centralArgumentHeight(KDFont::Size font) {
  ...
  if (this == last) {
    ...
    KDCoordinate differentialHeight =
        differentialLayout()->layoutSize(font).height();
