...
class VerticalOffsetLayoutNode final : public LayoutNode {
 ...
 public:
  enum class VerticalPosition : uint8_t { Subscript = 0, Superscript };
  enum class HorizontalPosition : uint8_t { Prefix = 0, Suffix };

  VerticalOffsetLayoutNode(
      VerticalPosition verticalPosition = VerticalPosition::Superscript,
      HorizontalPosition horizontalPosition = HorizontalPosition::Suffix)
      : LayoutNode(),
  ...
  void logNodeName(std::ostream &stream) const override {
    stream << (verticalPosition() == VerticalPosition::Subscript
                   ? "Subscript"
                   : "Superscript");
  }
 ...
 private:
  ...
  int baseOffsetInParent() const {
    return m_horizontalPosition == HorizontalPosition::Prefix ? +1 : -1;
  }
...
class VerticalOffsetLayout final : public OLayout {
 public:
  ...
  VerticalOffsetLayoutNode::VerticalPosition verticalPosition() const {
    return node()->verticalPosition();
  }
  VerticalOffsetLayoutNode::HorizontalPosition horizontalPosition() const {
    return node()->horizontalPosition();
  }
  bool setEmptyVisibility(EmptyRectangle::State state) {
    return node()->setEmptyVisibility(state);
  }
  bool isSuffixSuperscript() const {
