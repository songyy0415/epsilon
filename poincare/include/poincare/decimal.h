...
class DecimalNode final : public NumberNode {
 ...
 public:
  DecimalNode(const native_uint_t* mantissaDigits, uint8_t mantissaSize,
   ...
   * exponent: 3) are strictly ordered with the SimplificationOrder although
   * they are equal with the usual math order (1.000E3 == 1E3). */
...
class Decimal final : public Number {
 ...
 public:
  ...
  static Decimal Builder(const char* integralPart, int integralPartLength,
                         const char* fractionalPart, int fractionalPartLength,
                         int exponent);
  static Decimal Builder(Integer m, int e);
  template <typename T>
  static Decimal Builder(T f);
  static int Exponent(const char* integralPart, int integralPartLength,
                      const char* fractionalPart, int fractionalPartLength,
