...
class PointOfInterestNode : public TreeNode {
 ...
 public:
  PointOfInterestNode(double abscissa, double ordinate,
                      typename Solver<double>::Interest interest, uint32_t data,
  ...
  void logAttributes(std::ostream &stream) const override {
    stream << " abscissa=\"" << m_abscissa << "\" ordinate=\"" << m_ordinate
           << "\" data=\"" << m_data << "\" interest=\""
           << static_cast<int>(m_interest) << "\" inverted=\"" << m_inverted
...
static_assert(
    sizeof(PointOfInterestNode) <= 40,
    "merge m_subCurveIndex and m_data if you need more than one byte");
...
class PointOfInterest : public TreeHandle {
 public:
  ...
  double x() const {
    return node()->m_inverted ? node()->m_ordinate : node()->m_abscissa;
  }
  double y() const {
    return node()->m_inverted ? node()->m_abscissa : node()->m_ordinate;
  }
  int subCurveIndex() const { return node()->m_subCurveIndex; }
  typename Solver<double>::Interest interest() const {
    return node()->m_interest;
  }
 ...
 private:
  PointOfInterestNode *node() const {
    return static_cast<PointOfInterestNode *>(TreeHandle::node());
  }
};
