...
void assert_expression_simplify_to_with_dependencies(
    ...
    const char *const (&dependencies)[N], ReductionTarget target = User,
  ...
  if (N == 1 && dependencies[0][0] == '\0') {
    assert_parsed_expression_simplify_to(
        expression, simplifiedExpression, target, angleUnit, unitFormat,
        complexFormat, symbolicComputation, unitConversion);
    return;
  }
  ...
  quiz_assert_print_if_failure(d.otype() == ExpressionNode::Type::Dependency,
                               expression);
  assert_expression_serializes_to(d.childAtIndex(0), simplifiedExpression);
  OExpression m = d.childAtIndex(1);
  quiz_assert_print_if_failure(m.otype() == ExpressionNode::Type::OList,
                               expression);
...
QUIZ_CASE(poincare_dependency_derivative) {
  ...
  assert_expression_simplify_to_with_dependencies("diff(cos(x)+f(y),x,0)", "0",
                                                  {"y"});
  assert_expression_simplify_to_with_dependencies("diff(cos(x)+f(y),x,x)",
                                                  "-sin(x)", {"y", "cos(x)"});
  assert_reduce_and_store("1/0→y");
  assert_expression_simplify_to_with_dependencies("diff(cos(x)+f(y),x,0)",
                                                  Undefined::Name(), {""});
  Ion::Storage::FileSystem::sharedFileSystem->recordNamed("f.func").destroy();
...
QUIZ_CASE(poincare_dependency_parametered_expression) {
  ...
  assert_expression_simplify_to_with_dependencies(
      "int(f(x)+f(a),x,0,1)", "int(\u0014dep(2,{x,a}),x,0,1)", {""});
  assert_reduce_and_store("1/0→a");
  assert_expression_simplify_to_with_dependencies("int(f(x)+f(a),x,0,1)",
                                                  Undefined::Name(), {""});
  Ion::Storage::FileSystem::sharedFileSystem->recordNamed("f.func").destroy();
  Ion::Storage::FileSystem::sharedFileSystem->recordNamed("a.exp").destroy();
  ...
  assert_expression_simplify_to_with_dependencies(
      "int(diff(f(x),x,x),x,0,1)", "int(\u0014dep(1,{x}),x,0,1)", {""});
  Ion::Storage::FileSystem::sharedFileSystem->recordNamed("f.func").destroy();
...
QUIZ_CASE(poincare_dependency_power) {
  ...
  assert_expression_simplify_to_with_dependencies("(1+x)/(1+x)", "1",
                                                  {"1/\u0012x+1\u0013"});
...
QUIZ_CASE(poincare_dependency_trigonometry) {
  assert_expression_simplify_to_with_dependencies(
      "arctan(1/x)", "\u0012π×sign(x)-2×arctan(x)\u0013/2", {"1/x"});
}
