...
QUIZ_CASE(pcj_polynomial_parsing) {
  assert_polynomial_is_parsed(
      ...
      /* polynomial */  // TODO KPol(exponentsPi, π_e, 1_e, KPol(exponentsE0,
                        // "e"_e, 3_e), KPol(exponentsE1, "e"_e, 3_e),
                        // KPol(exponentsE2, "e"_e, 1_e))
                        /* polynomial */
      KPol(Exponents<3, 2, 1, 0>(), π_e, 1_e,
           KPol(Exponents<1>(), "e"_e, KMult(3_e, 1_e, 1_e)),
           KPol(Exponents<2>(), "e"_e, KMult(3_e, 1_e, 1_e)),
           KPol(Exponents<3>(), "e"_e, 1_e)));
...
QUIZ_CASE(pcj_polynomial_operations) {
  ...
  // TODO once basicReduction implemented
  // assert_trees_are_equal(Polynomial::KAddition(polA, polB),
  // EditionReference(KPol(Exponents<3, 2, 1, 0>(), "x"_e, 1_e, 1_e,
  // KPol(Exponents<2, 1>(), "y"_e, 2_e, 10_e), KPol(Exponents<1, 0>(), "y"_e,
  // 1_e, 24_e))));
  assert_trees_are_equal(
      Polynomial::Addition(EditionReference(polA), EditionReference(polB)),
      EditionReference(
          KPol(Exponents<3, 2, 1, 0>(), "x"_e, 1_e, 1_e,
               KPol(Exponents<2, 1>(), "y"_e, 2_e, KAdd(3_e, 7_e)),
               KPol(Exponents<1, 0>(), "y"_e, 1_e, KAdd(1_e, 23_e)))));
  CachePool::sharedCachePool()->editionPool()->flush();

  /* A * B = x^5 + 3yx^4 + (2y^2+8y+1)*x^3 + (6y^3+21y^2+23)x^2 + (2y^3+9y^2+
   * 76y)x + 23y + 23 */
  // TODO once basicReduction
  // assert_trees_are_equal(Polynomial::KMultiplication(EditionReference(polA),
  // EditionReference(polB)), EditionReference(KPol(Exponents<5, 4, 3, 2, 1,
  // 0>(), "x"_e, 1_e, KPol(Exponents<1>(), "y"_e, 3_e), KPol(Exponents<2, 1,
  // 0>(), "y"_e, 2_e, 8_e, 1_e), KPol(Exponents<3, 2, 0>(), "y"_e, 6_e, "21"_e,
  // 23_e), KPol(Exponents<3, 2, 1>(), "y"_e, 2_e, 9_e, 76_e, KPol(Exponents<1,
  // 0>(), "y"_e, 23_e, 23_e))));
  assert_trees_are_equal(
      Polynomial::Multiplication(EditionReference(polA),
                                 EditionReference(polB)),
      EditionReference(KPol(
          Exponents<5, 4, 3, 2, 1, 0>(), "x"_e, KMult(1_e, 1_e),
          KPol(Exponents<1>(), "y"_e, KMult(3_e, 1_e)),
          KPol(Exponents<2, 1, 0>(), "y"_e, KMult(2_e, 1_e),
               KAdd(KMult(1_e, 1_e), KMult(7_e, 1_e)), KMult(1_e, 1_e)),
          KPol(Exponents<3, 2, 0>(), "y"_e, KMult(3_e, 2_e), KMult(3_e, 7_e),
               KMult(1_e, 23_e)),
          KPol(Exponents<3, 2, 1>(), "y"_e, KMult(1_e, 2_e),
               KAdd(KMult(1_e, 2_e), KMult(1_e, 7_e)),
               KAdd(KMult(1_e, 7_e), KMult(3_e, 23_e))),
          KPol(Exponents<1, 0>(), "y"_e, KMult(1_e, 23_e), KMult(1_e, 23_e)))));
  CachePool::sharedCachePool()->editionPool()->flush();
  ...
  // assert_trees_are_equal(quotient, KPol(Exponents<1, 0>(), "x"_e,
  // KPol(Exponents<1>(), "y"_e, 1_e), KSub(0_e, 1_e)));
  // assert_trees_are_equal(remainder, KPol(Exponents<1, 0>(), "x"_e, 1_e,
  // 1_e));
}
