...
void testPolynomialOperations() {
  /* A = x^2 + 3*x*y + y + 1 */
  EditionReference polA = EditionReference(Pol({2, 1, 0}, Symb("x"), 1_sn, Pol({1},  Symb("y"), 3_n), Pol({1, 0}, Symb("y"), 1_sn, 1_sn)));
  /* B = x^3 + 2*x*y^2 + 7*x*y + 23 */
  EditionReference polB = EditionReference(Pol({3, 1, 0}, Symb("x"), 1_sn, Pol({2, 1},  Symb("y"), 2_sn, 7_n), 23_n));
  ...
  assert_trees_are_equal(Polynomial::Addition(polA, polB), EditionReference(Pol({3, 2, 1, 0}, Symb("x"), 1_sn, 1_sn, Pol({2, 1}, Symb("y"), 2_sn, Add(3_n, 7_n)), Pol({1, 0}, Symb("y"), 1_sn, Add(1_sn, 23_n)))));
}
