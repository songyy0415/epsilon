...
void IntersectionGraphController::reloadBannerView() {
  ...
  numberOfChar += g->nameWithArgument(
      buffer + numberOfChar, bufferSize - numberOfChar - strlen(legend));
  if (numberOfChar - numberOfCharsBeforeGName == fNameLength &&
      memcmp(buffer, buffer + numberOfCharsBeforeGName, fNameLength) == 0) {
