 ...
 protected:
  ...
  class Pattern {
   public:
    ...
    Pattern(KDColor c0, KDColor c1, KDColor c2, KDColor c3,
            KDColor cBackground = k_transparent);
    Pattern(KDColor c = k_transparent) : Pattern(c, c, c, c, c) {}
    Pattern(bool s0, bool s1, bool s2, bool s3, KDColor color,
  ...
  class CurveDrawing {
   ...
   private:
    /* When iterating between two abscissas, the steepest curve can go from
     * bottom to top of screen in one pixel of width. By computing a maximum of
     * 2^8 dots, we ensure to compute at least one value per pixel, since 2^8 >
     * screen height Note: this is not true if the curve goes outside of the
     * screen though.
     */
