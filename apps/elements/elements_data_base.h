...
class ElementsDataBase {
 public:
  ...
  static bool IsElement(AtomicNumber z) {
    return z > k_noElement && z <= k_numberOfElements;
  }
  static bool IsMetal(AtomicNumber z) {
    return Group(z) < ElementData::Group::Metalloid;
  }

  static double MolarMass(AtomicNumber z) {
    return DataForElement(z)->molarMass();
  }
  static double Electronegativity(AtomicNumber z) {
    return DataForElement(z)->electronegativity();
  }
  static double Radius(AtomicNumber z) { return DataForElement(z)->radius(); }
  static double MeltingPoint(AtomicNumber z) {
    return DataForElement(z)->meltingPoint();
  }
  static double BoilingPoint(AtomicNumber z) {
    return DataForElement(z)->boilingPoint();
  }
  static double Density(AtomicNumber z) { return DataForElement(z)->density(); }
  static double Affinity(AtomicNumber z) {
    return DataForElement(z)->affinity();
  }
  static double EnergyOfIonization(AtomicNumber z) {
    return DataForElement(z)->energyOfIonization();
  }
  static const char* Symbol(AtomicNumber z) {
    return DataForElement(z)->symbol();
  }
  static I18n::Message Name(AtomicNumber z) {
    return DataForElement(z)->name();
  }
  static uint16_t NumberOfMass(AtomicNumber z) {
    return DataForElement(z)->numberOfMass();
  }
  static ElementData::Group Group(AtomicNumber z) {
    return DataForElement(z)->group();
  }
  static ElementData::PhysicalState PhysicalState(AtomicNumber z) {
    return DataForElement(z)->physicalState();
  }
  static ElementData::Block Block(AtomicNumber z) {
    return DataForElement(z)->block();
  }
