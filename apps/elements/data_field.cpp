...
DataField::ColorPair DoubleDataField::getColors(AtomicNumber z) const {
  ...
  return ColorPair(blendAlphaForContinuousParameter(z), min.fg(), max.fg(),
                   min.bg(), max.bg());
}

uint8_t DoubleDataField::blendAlphaForContinuousParameter(
  ...
  return static_cast<uint8_t>(numberOfLowerValues * ((1 << 8) - 1.f) /
...
int maxPopulation(int l) { return 2 * (2 * l + 1); }
...
I18n::Message MetalDataField::protectedGetMessage(AtomicNumber z) const {
  ElementData::Group group = ElementsDataBase::Group(z);
  if (ElementsDataBase::IsMetal(z) || group == ElementData::Group::Metalloid ||
      group == ElementData::Group::Unidentified) {
...
DataField::ColorPair MetalDataField::getColors(AtomicNumber z) const {
  ...
  constexpr ColorPair k_colors[] = {
      ...
      ColorPair(Palette::MetallicVerdigrisDark,
                Palette::MetallicVerdigrisLight),
      ColorPair(Palette::MetallicTurquoiseDark,
                Palette::MetallicTurquoiseLight),
      ColorPair(Palette::MetallicAquamarineDark,
...
Layout AffinityDataField::getLayout(AtomicNumber z,
                                    int significantDigits) const {
