...
double InterestData::defaultValue(uint8_t param) const {
  ...
  constexpr double k_defaultValues[k_numberOfSharedDoubleValues] = {
      100.0, 1.0, -1000.0, 2.7};
...
double CompoundInterestData::defaultValue(uint8_t param) const {
  ...
  constexpr double
      k_defaultValues[k_numberOfDoubleValues - k_numberOfSharedDoubleValues] = {
          0.0, 12.0, 12.0};
...
double computeB(double i, double N) { return std::pow(1.0 + i, -N); }

double computeA(double i, double S, double b, double N) {
  return i == 0.0 ? N : (1.0 + i * S) * (1.0 - b) / i;
}

double computeN(double rPct, double PV, double Pmt, double FV, double S,
                double i) {
  if (rPct == 0.0) {
    return -(PV + FV) / Pmt;
  }
...
double computeRPct(double N, double PV, double Pmt, double FV, double PY,
                   double CY, double S) {
  if (Pmt == 0.0) {
  ...
  Poincare::Internal::Solver<double> solver(-100., 100.);
...
void CompoundInterestData::setValue(uint8_t param, double value) {
  ...
  if (param == static_cast<uint8_t>(Parameter::PY)) {
    ...
    m_values[static_cast<uint8_t>(Parameter::CY) -
             k_numberOfSharedDoubleValues] = value;
  }
