...
bool OneMean::TestInitializeDistribution(Statistic* statistic,
...
bool OneMean::IntervalInitializeDistribution(
    Statistic* statistic, DistributionType distributionType) {
  if (!SignificanceTest::InitializeDistribution(statistic, distributionType)) {
    return false;
  }
  statistic->~Statistic();
...
void OneMean::ComputeZTest(Test* t) {
  ...
  t->m_testCriticalValue =
      ComputeTZ(mean, X(t->parametersArray()), S(t->parametersArray()),
                N(t->parametersArray()));
...
ParameterRepresentation OneProportion::ParameterRepresentationAtIndex(
    int index) {
  switch (index) {
    case ParamsOrder::x: {
...
bool TwoMeans::TestInitializeDistribution(Statistic* statistic,
                                          DistributionType distributionType) {
  if (!SignificanceTest::InitializeDistribution(statistic, distributionType)) {
    return false;
  }
...
bool TwoMeans::IntervalInitializeDistribution(
    Statistic* statistic, DistributionType distributionType) {
  if (!SignificanceTest::InitializeDistribution(statistic, distributionType)) {
    return false;
  }
...
bool TwoMeans::TValidateInputs(double* params) {
  return (ComputeDegreesOfFreedom(S1(params), N1(params), S2(params),
                                  N2(params)) > 0) &&
         ZValidateInputs(params);
}
...
void TwoMeans::ComputeZTest(Test* t) {
  ...
  t->m_testCriticalValue =
      ComputeTZ(deltaMean, X1(t->parametersArray()), N1(t->parametersArray()),
                S1(t->parametersArray()), X2(t->parametersArray()),
                N2(t->parametersArray()), S2(t->parametersArray()));
  t->m_pValue = ComputePValue(t);
}

void TwoMeans::ComputeTTest(Test* t) {
  t->m_degreesOfFreedom = ComputeDegreesOfFreedom(
      S1(t->parametersArray()), N1(t->parametersArray()),
      S2(t->parametersArray()), N2(t->parametersArray()));
  ComputeZTest(t);
}
...
void TwoMeans::ComputeTInterval(Interval* i) {
  i->m_degreesOfFreedom = ComputeDegreesOfFreedom(
      S1(i->parametersArray()), N1(i->parametersArray()),
      S2(i->parametersArray()), N2(i->parametersArray()));
  ComputeZInterval(i);
}

double TwoMeans::ComputeTZ(double deltaMean, double meanSample1, double n1,
                           double sigma1, double meanSample2, double n2,
                           double sigma2) {
  return ((meanSample1 - meanSample2) - (deltaMean)) /
         ComputeStandardError(sigma1, n1, sigma2, n2);
}

double TwoMeans::ComputeStandardError(double s1, double n1, double s2,
                                      double n2) {
  return OMG::LaxToZero(std::sqrt((s1 * s1 / n1 + s2 * s2 / n2)));
}

double TwoMeans::ComputeDegreesOfFreedom(double s1, double n1, double s2,
                                         double n2) {
...
ParameterRepresentation TwoProportions::ParameterRepresentationAtIndex(
    int index) {
  switch (index) {
    case ParamsOrder::x1: {
...
void TwoProportions::ComputeTest(Test* t) {
  ...
  t->m_testCriticalValue =
      ComputeZ(deltaP0, X1(t->parametersArray()), N1(t->parametersArray()),
               X2(t->parametersArray()), N2(t->parametersArray()));
  t->m_pValue = ComputePValue(t);
}
...
double TwoProportions::ComputeStandardError(double x1, double n1, double x2,
                                            double n2) {
  double p1Estimate = x1 / n1;
  double p2Estimate = x2 / n2;
  return OMG::LaxToZero(std::sqrt(p1Estimate * (1 - p1Estimate) / n1 +
                                  p2Estimate * (1 - p2Estimate) / n2));
}
