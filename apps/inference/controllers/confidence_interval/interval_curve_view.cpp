...
static void convertFloatToText(float f, char* buffer, int bufferSize) {
  Poincare::PrintFloat::ConvertFloatToText<float>(
      f, buffer, IntervalAxis::k_bufferSize, IntervalAxis::k_glyphLength,
      PlotPolicy::AbstractLabeledAxis::k_numberSignificantDigits,
...
void IntervalAxis::reloadAxis(AbstractPlotView* plotView, OMG::Axis axis) {
  ...
  /* Memoize the interval bounds, as at the time of display, there is no
   * guarantee the bounds will be correct. */
  m_ticks[0] = low;
  m_ticks[1] = high;
  ...
  if (spaceBetweenBounds > 0 &&
      spaceBetweenBounds <
          labelMaxWidth + KDFont::GlyphWidth(AbstractPlotView::k_font)) {
...
void IntervalPlotPolicy::drawPlot(const AbstractPlotView* plotView,
  ...
  for (int i = 0; i < Interval::k_numberOfDisplayedIntervals; i++) {
    ...
    constexpr int k_ThresholdBufferSize =
        PrintFloat::charSizeForFloatsWithPrecision(
            PlotPolicy::AbstractLabeledAxis::k_numberSignificantDigits) +
        1;  // 1 = strlen("%")
    ...
    Print::CustomPrintf(
        ...
        PlotPolicy::AbstractLabeledAxis::k_numberSignificantDigits);
    KDColor textColor =
        isMainInterval ? KDColorBlack : Escher::Palette::GrayDarkMiddle;
    plotView->drawLabel(
        ctx, rect, buffer, Coordinate2D<float>(estimate, verticalPosition),
        AbstractPlotView::RelativePosition::There,
        AbstractPlotView::RelativePosition::Before, textColor, true);

    // Draw the interval segment : --------
    double intervalRightBound = estimate - marginOfError;
    double intervalLeftBound = estimate + marginOfError;
    KDColor intervalColor = isMainInterval ? Escher::Palette::YellowDark
