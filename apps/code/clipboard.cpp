...
const UTF8Helper::TextPair* Clipboard::PythonTextPairs() {
   ...
   * "3××3". */
  constexpr static UTF8Helper::TextPair
      k_pythonTextPairs[k_numberOfPythonTextPairs] = {
          ...
          /* Since TextPairs are also used to pair events, we need to keep both
           * ^2 and ^ to get the desired behavior in python when using power or
           * square key. */
...
void Clipboard::replaceCharForPython(bool entersPythonApp) {
  ...
  UTF8Helper::TryAndReplacePatternsInStringByPatterns(
      m_textBuffer, TextField::MaxBufferSize(), PythonTextPairs(),
      k_numberOfPythonTextPairs, entersPythonApp);
}
