...
void assert_finite_integral_between_abscissas_is(
    Distributions::Distribution* distribution, double a, double b,
    double result) {
...
QUIZ_CASE(distributions_results_binomial) {
  ...
  assert_roughly_equal<float>(
      distribution.evaluateAtAbscissa(4),
      std::pow(1 - 0.6, 32 - 4) * std::pow(0.6, 4) * 35960 /* 32 choose 4 */,
      1e-4);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 19.0,
      0.53819340022502382137048471122398041188716888427734375);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 10.0,
      0.00094770581410635293569122428181117356871254742145538330078125);
  ...
  assert_roughly_equal<float>(
      distribution.evaluateAtAbscissa(4),
      std::pow(1 - 0.1, 17 - 4) * std::pow(0.1, 4) * 2380 /* 17 choose 4 */,
      1e-5);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 0.0,
      0.166771816996665822596668249389040283858776092529296875);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 1.0,
      0.4817852491014791294077213024138472974300384521484375);
  ...
  assert_roughly_equal<float>(
      distribution.evaluateAtAbscissa(4),
      std::pow(1 - 0.2, 21 - 4) * std::pow(0.2, 4) * 5985 /* 21 choose 4 */,
      1e-5);
  assert_finite_integral_between_abscissas_is(&distribution, 4.0, 4.0,
                                              0.21563235015849934848);
  assert_finite_integral_between_abscissas_is(&distribution, 5.0, 4.0, 0.0);
  assert_finite_integral_between_abscissas_is(&distribution, 4.0, 5.0,
...
QUIZ_CASE(distributions_results_uniform) {
  ...
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 0.23f, (1.f + .23f) / 2.f);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, -0.9, (1.F - 0.9f) / 2.f);

  // U(0.1, 17)
  distribution.setParameterAtIndex(0.1, 0);
  distribution.setParameterAtIndex(17., 1);
  assert_roughly_equal<float>(distribution.evaluateAtAbscissa(4),
                              1.f / (17.f - .1f));
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 0.2 * (17. - 0.1) + 0.1, 0.2);
...
QUIZ_CASE(distributions_results_exponential) {
  ...
  assert_roughly_equal<float>(distribution.evaluateAtAbscissa(1),
                              std::exp(-1.f));
  assert_roughly_equal<float>(distribution.evaluateAtAbscissa(14),
                              std::exp(-14.f));
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 0.23f, 1 - std::exp(-.23f));
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 2.f, 1 - std::exp(-2.f));

  // EXP(13)
  distribution.setParameterAtIndex(.2f, 0);
  assert_roughly_equal<float>(distribution.evaluateAtAbscissa(2),
                              .2f * std::exp(-.2f * 2.f));
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 2.3, 1.f - std::exp(-.2f * 2.3f));
  assert_cumulative_distributive_function_direct_and_inverse_is(
...
QUIZ_CASE(distributions_results_chi_squared) {
  ...
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 1.3,
      0.7457867763960355222963016785797663033008575439453125);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 2.9874567,
      0.91608813616541218127764523160294629633426666259765625);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 4.987,
...
QUIZ_CASE(distributions_results_student) {
  ...
  distribution.setParameterAtIndex(1.0, 0);
  assert_roughly_equal<float>(distribution.evaluateAtAbscissa(0.1),
                              0.31515830315226806f);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, -2.0,
      0.1475836176504332741754010762247405259511345238869178945999223128627);
  assert_cumulative_distributive_function_direct_and_inverse_is(&distribution,
                                                                0.0, 0.5);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 3.4567,
      0.9103622230304611040452300585457123816013336181640625);

  // Student distribution with 1.3 degrees of freedom
  distribution.setParameterAtIndex(1.3, 0);
  assert_roughly_equal<float>(distribution.evaluateAtAbscissa(0.1),
                              0.33047714482602075f);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, -4.987,
      0.041958035438260647687069848643659497611224651336669921875);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 1.3,
      0.8111769751474751100062121622613631188869476318359375);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 2.9874567,
      0.92143912473359812498330256858025677502155303955078125);

  // Student distribution with 5.4 degrees of freedom
  distribution.setParameterAtIndex(5.4, 0);
  assert_roughly_equal<float>(distribution.evaluateAtAbscissa(0.1),
                              0.3787417926621869f, 1e-6);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, -4.987,
      0.00167496657737900025118837898929768925881944596767425537109375);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 1.3,
      0.876837383157582639370275501278229057788848876953125);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 2.9874567,
      0.98612148076325445433809591122553683817386627197265625);

  // Student distribution with 6 degrees of freedom
  distribution.setParameterAtIndex(6.0, 0);
  assert_roughly_equal<float>(distribution.evaluateAtAbscissa(0.1),
                              0.38050851123793844f);
  assert_finite_integral_between_abscissas_is(&distribution, 4.0, 4.0, 0.0);
  assert_finite_integral_between_abscissas_is(&distribution, 5.0, 4.0, 0.0);
  assert_finite_integral_between_abscissas_is(&distribution, 4.0, 5.0,
...
QUIZ_CASE(distributions_results_fisher) {
  ...
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 3.3, 0.6796445888753);
  ...
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 3.0, 0.7400733003272);
  ...
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 1.4, 0.94560850441205857);
  assert_cumulative_distributive_function_direct_and_inverse_is(
      &distribution, 1.425, 0.95425004959692871775);
