...
void EquationListView::layoutSubviews(bool force) {
  setChildFrame(&m_listView, KDRect(0, 0, bounds().width(), bounds().height()),
                force);
  if (m_braceStyle != BraceStyle::None) {
    ...
    KDCoordinate scrollBraceHeight =
        m_listView.minimalSizeForOptimalDisplay().height() - offset().y();
    scrollBraceHeight = m_braceStyle == BraceStyle::OneRowShort
                            ? scrollBraceHeight - Metric::StoreRowHeight
                            : scrollBraceHeight;
    setChildFrame(&m_scrollBraceView,
                  KDRect(0, 0, k_braceTotalWidth, scrollBraceHeight), force);
  } else {
    setChildFrame(&m_scrollBraceView, KDRectZero, force);
  }
}
...
const uint8_t middleBrace[braceCenterHeight][braceCenterWidth] = {
    {0xFF, 0xFF, 0xFF, 0x00}, {0xFF, 0xFF, 0xD7, 0x29},
    {0xFF, 0xFF, 0xB9, 0x33}, {0xFF, 0xFF, 0x7B, 0x76},
    {0xFF, 0xFF, 0x2C, 0xC3}, {0xFF, 0xAA, 0x44, 0xFF},
    {0xFF, 0x38, 0xAB, 0xFF}, {0x00, 0x00, 0xFF, 0xFF},
    {0xFF, 0x38, 0xAB, 0xFF}, {0xFF, 0xAA, 0x44, 0xFF},
    {0xFF, 0xFF, 0x2C, 0xC3}, {0xFF, 0xFF, 0x7B, 0x76},
    {0xFF, 0xFF, 0xB9, 0x33}, {0xFF, 0xFF, 0xD7, 0x29},
    {0xFF, 0xFF, 0xFF, 0x00},
};
