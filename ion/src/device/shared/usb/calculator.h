...
class Calculator : public Device {
      ...
      : Device(&m_dfuInterface),
        m_deviceDescriptor(
            0x0210, /* bcdUSB: USB Specification Number which the device
                     * complies to. Must be greater than 0x0200 to use the BOS.
                     */
 ...
 protected:
  ...
  uint8_t getActiveConfiguration() override { return k_bConfigurationValue; }
 ...
 private:
  ...
  bool getExtendedCompatIDCommand(uint8_t* transferBuffer,
                                  uint16_t* transferBufferLength,
                                  uint16_t transferBufferMaxLength);
