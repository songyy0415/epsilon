...
parser.add_argument(
    "-o", type=str, required=True, help="The implementation file to output"
)
...
def print_background_areas(f, background_values):
    ...
    f.write(
        "constexpr static int backgroundHeight = " + str(background_area[3]) + ";\n"
    )
...
def print_key(f, key_values, shape):
    f.write(
        "  KeyLayout("
        + str(round(key_values[0] + key_values[2] / 2))
        + ", "
        + str(round(key_values[1] + key_values[3] / 2))
        + ", KeyLayout::Shape::"
        + shape
        + "),\n"
    )
