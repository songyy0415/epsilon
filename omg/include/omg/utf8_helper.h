...
class TextPair {
 public:
  ...
  constexpr TextPair(const char* firstString, const char* secondString,
                     bool removeParenthesesExtension = false,
                     ReplacementRule rule = nullptr)
      : m_firstString(firstString),
        m_secondString(secondString),
        m_removeParenthesesExtension(removeParenthesesExtension),
        m_rule(rule) {}
  ...
  bool shouldReplace(const char* s, int length, int position) {
    return m_rule ? m_rule(s, length, position) : true;
  }
...
void RemoveCodePoint(char* buffer, CodePoint c,
 ...
 * - firstToSecond defines if replace the first string of a TextPair by the
...
/* Copy src into dst until end of dst or code point c, with null termination.
 ...
 *
 * */
typedef void (*CodePointAction)(int codePointOffset, void* contextPointer,
                                int contextInt1, int contextInt2);
const char* PerformAtCodePoints(
    const char* string, CodePoint c, CodePointAction actionCodePoint,
    CodePointAction actionOtherCodePoint, void* contextPointer, int contextInt1,
    int contextInt2 = -1, CodePoint stoppingCodePoint = UCodePointNull,
    bool goingRight = true, const char* initialPosition = nullptr,
    const char* stoppingPosition = nullptr);
...
int RemovePreviousGlyph(const char* text, char* location,
                        CodePoint* c = nullptr);
...
void CountGlyphsInLine(const char* text, int* before, int* after,
                       const char* beforeLocation,
                       const char* afterLocation = nullptr);
...
const char* SuffixCaseInsensitiveNoCombining(const char* prefix,
                                             const char* text);
